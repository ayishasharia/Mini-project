        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-8 col-sm-12">
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM product1";
                    $result = mysqli_query($link,$total_pages_sql);
                    $total_rows = mysqli_fetch_array($result)[0];
                    $total_pages = ceil($total_rows / $no_of_records_per_page);

                    //Column sorting on column name
                    $orderBy = array('catid', 'name', 'image', 'price');
                    $order = 'pid';
                    ...
                    // Attempt select query execution
						...
						{
							$cat=$_GET['catid'];
							$sql = "SELECT * FROM product1 where catid=".$cat;
						}
                  else{
						$sql = "SELECT * FROM product1 ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
				  }
                    $count_pages = "SELECT * FROM product1";


                    if(!empty($_GET['search'])) {
                        $search = ($_GET['search']);
                        $sql = "SELECT * FROM product1
                            WHERE CONCAT_WS (catid,name,image,price)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                    ...
                    if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                echo "<thead>";
                                    echo "<tr>";
                                        echo "<th><a href=?search=$search&sort=&order=catid&sort=$sort>Category</th>";
										echo "<th><a href=?search=$search&sort=&order=name&sort=$sort>Product Name</th>";
										echo "<th><a href=?search=$search&sort=&order=image&sort=$sort>Image</th>";
										echo "<th><a href=?search=$search&sort=&order=price&sort=$sort>Price (Rs)</th>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . $row['catid'] . "</td>";echo "<td>" . $row['name'] . "</td>";
                                    ...
                                    $img2= $row['image2'];
                                    echo "<td><img src='" . $img2 . "' width='100'></td>";
									
									
									echo "<td>" . $row['price'] . "</td>";
                                        echo "<td>";
                                         echo "<a href='product1-delete.php?pid=". $row['pid'] ."' title='Delete Record' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
