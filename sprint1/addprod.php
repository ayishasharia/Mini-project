	...
	$fnn=date("YmdHis").".".$ext;
	 $target_dir = "uploads/";
      ...
      if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
        ...
        $vars = parse_columns('product1', $_POST);
        $stmt = $pdo->prepare("INSERT INTO product1 (catid,name,image,price,description) VALUES (?,?,?,?,?)");

        if($stmt->execute([ $catid,$name,$target_file,$price,$description  ])) {
                $stmt = null;
                                ...
                                <label>Category</label>
                                    ...
                                    <?php
                                        ...
                                        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            ...
                                            if ($row["catid"] == $catid){
                                            echo '<option value="' . "$row[catid]" . '"selected="selected">' . "$value" . '</option>';
                                            } else {
                                                echo '<option value="' . "$row[catid]" . '">' . "$value" . '</option>';
                                        }
                                        }
                                    ?>
                                    </select>
                                <span class="form-text"><?php echo $catid_err; ?></span>
                            </div>
						<div class="form-group">
