...
if(isset($_GET["Order_id"]) && !empty($_GET["Order_id"])){
    ...
    $stmt = $pdo->prepare("UPDATE order_tbl SET status=? WHERE Order_id=?");
    ...
    if(!$stmt->execute([ $status,$Order_id  ])) {
		...
		//praveen
		header("location: order_tbl-index.php");
    }
} else {
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group">
                                <label>User</label>
                                    ...
                                    <?php
                                        $sql = "SELECT *,uid FROM registration";
                                        $result = mysqli_query($link, $sql);
                                        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            array_pop($row);
                                            $value = implode(" | ", $row);
                                            if ($row["uid"] == $u_id){
                                            echo '<option value="' . "$row[uid]" . '"selected="selected">' . "$value" . '</option>';
                                            } else {
                                                echo '<option value="' . "$row[uid]" . '">' . "$value" . '</option>';
                                        }
                                        }
                                    ?>
                                    </select>
						...
						<div class="form-group">
                                <label>Name</label>
                                <input type="text" name="Name" maxlength="20"class="form-control" value="<?php echo $Name; ?>">
                                ...
                                <label>pid</label>
                                    ...
                                    <?php
                                        ...
                                        while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
                                            ...
                                            $value = implode(" | ", $row);
                                            if ($row["pid"] == $pid){
                                            echo '<option value="' . "$row[pid]" . '"selected="selected">' . "$value" . '</option>';
                                            } else {
                                                echo '<option value="' . "$row[pid]" . '">' . "$value" . '</option>';
                                        }
                                        }
                                    ?>
                                    </select>
						...
						<div class="form-group">
                                <label>d_date</label>
                                <input type="date" name="d_date" class="form-control" value="<?php echo $d_date; ?>">
						...
						<div class="form-group">
                                <label>status</label>
                                <input type="text" name="status" maxlength="50"class="form-control" value="<?php echo $status; ?>">
                                <span class="form-text"><?php echo $status_err; ?></span>
                            </div>

                        <input type="hidden" name="Order_id" value="<?php echo $Order_id; ?>"/>
                        <input type="submit" class="btn btn-primary" value="Submit">
                        <a href="order_tbl-index.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        </div>
